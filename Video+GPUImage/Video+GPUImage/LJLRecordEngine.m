...
#import "LJLRecordEncoder.h"


@interface LJLRecordEngine ()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate,CAAnimationDelegate>
{
...
@property (nonatomic, strong) LJLRecordEncoder *recordEncoder;
...
@implementation LJLRecordEngine
...
- (void)movieToImageHandler:(void (^)(UIImage *movieImage))handler {
    NSURL *url = [NSURL fileURLWithPath:self.videPath];
...
- (void)changeMovToMp4:(NSURL *)mediaURL dataBlock:(void (^)(UIImage *movieImage))handler {
    ...
    //视频路径
    self.videPath = [basePath stringByAppendingPathComponent:[self getUploadFileType:@"video" fileType:@"mp4"]];
    exportSession.outputURL = [NSURL fileURLWithPath:self.videPath];
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    @synchronized(self) {
        ...
        if ((self.recordEncoder == nil) && !isVideo) {
            ...
            self.videPath = [[self getVideoCachePath] stringByAppendingPathComponent:videoName];
            self.recordEncoder = [LJLRecordEncoder encoderForPath:self.videPath
                                                          height:_height
                                                           width:_width
                                                        channels:_channels
                                                         samples:_samplerate];
